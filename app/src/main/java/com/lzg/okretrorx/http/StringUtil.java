...
public final class StringUtil {
    ...
    public static String insertBreakingWhitespace(int lineLen, String original) {
        ...
        while (length - currPos > lineLen) {
            retval.append(original, currPos, currPos + lineLen);
            currPos += lineLen;
            retval.append(" ");
        }
        retval.append(original, currPos, length);
    ...
    public static boolean isNotEmptyAndNull(String s) {
        return isNotEmpty(s) && !equals(s, "null");
    }
    ...
    public static boolean isEmptyOrZero(String s) {
        //return makeSafe(s).length() == 0;
        return s == null || "0".equals(s) || s.length() == 0;
    }
...
//  }
     ...
     * 数组拼接字符串
     *
     * @param strings 需要拼接的数组
     * @param replace 拼接符
     * @param size    截取数组的前几个(-1表示全部截取)
     * @return result
     */
    public static String replaceToString(List<String> strings, String replace, int size) {
        StringBuilder result = new StringBuilder();
        if (strings == null || strings.size() == 0) {
            return result.toString();
        }
        if (0 < size && size <= strings.size()) {
            for (int i = 0; i < size; i++) {
                if (i == 0) {
                    result = new StringBuilder(strings.get(i));
                } else {
                    result.append(replace).append(strings.get(i));
                }
            }
        } else {
     ...
     * @return
     */
    public static String getNewString(String string, int count) {
        if (isNotEmpty(string) && string.length() > count) {
            return string.subSequence(0, count) + "...";
        }
        return string;
    }
